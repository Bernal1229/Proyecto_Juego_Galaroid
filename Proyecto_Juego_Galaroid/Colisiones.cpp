...
void colision_balas_meteoritos(std::vector<PtrBala>& Balas, std::vector<PtrEnemigo>& Enemigos, ALLEGRO_BITMAP*& Imagen1, ALLEGRO_BITMAP*& Imagen2, Estadisticas& stats, ALLEGRO_BITMAP* Imagenes[6], std::vector<PtrExplosion>& Explosiones) {
	...
	//En esta función se definen las colisiones de la bala con el meteorito, comparando las distancias entre las balas y los meteoritos.
...
//Esto se hace mediante la comparación de la suma de los radios de cada objeto.
...
void colision_jugador_meteoritos(Jugador jugador, std::vector<PtrEnemigo>& Enemigos, ALLEGRO_BITMAP*& Imagen1, ALLEGRO_BITMAP*& Imagen2, Estadisticas& stats) {
	...
	//Esta funcion funciona para revisar la colision entre el jugador y los meteoritos para así restar vidas. Tiene una logica similar a la colision entre balas y meteoritos.
